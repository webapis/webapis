...
export function useAuth() {
    ...
    function onLoginBlur(e) {
        ...
        switch (name) {
            case 'password':
                if (password === '') {
                    dispatch({ type: actionTypes.CONSTRAINT_VALIDATION, name: 'password', isValid: false, message: 'Required field' })
                }
                break;
            case 'emailorusername':
               // if (emailorusername === '') {
                    dispatch({ type: actionTypes.CONSTRAINT_VALIDATION, name: 'emailorusername', ...cv.validateEmailOrUsername({value:emailorusername}) })
                //}
                break;
            default:
