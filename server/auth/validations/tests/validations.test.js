...
describe("validateUsernameOrEmail", () => {
  it("isValidUsername true", () => {
    expect(validations.isValidUsername({ username: "tkmhousenew" })).toBe(true);
  });
  it("isValidUsername false", () => {
    expect(validations.isValidUsername({ username: "11" })).toBe(false);
  });
  it("isValidEmail true", () => {
    expect(validations.isValidEmail({ email: "tkmhousenew@gmail.com" })).toBe(
      true
    );
  });

  it("isValidEmail false", () => {
    expect(validations.isValidEmail({ email: "tkmhousenewgmail.com" })).toBe(
      false
    );
  });
