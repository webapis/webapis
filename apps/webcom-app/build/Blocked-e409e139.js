import { h } from "./index-ddb7f63d.js";
import "./style-04053a29.js";
import { B as Block } from "./Block-6e912fb7.js";

function Blocked({ contact, setRoute }) {
  const { username } = contact;
  return h(
    "div",
    {
      className: "chat-state-view",
      "data-testid": "blocked",
    },
    h(
      "div",
      {
        style: {
          flex: 10,
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
          flexDirection: "column",
          margin: 8, //  borderBottom: '4px solid white',
        },
      },
      h(Block, {
        color: "red",
        width: 50,
        height: 50,
      }),
      h("div", null, username),
      h(
        "p",
        {
          style: {
            textAlign: "center",
          },
        },
        "You blocked ",
        h("b", null, username && username),
        ".You cannot communicate with someone you blocked."
      )
    ),
    h(
      "div",
      {
        style: {
          display: "flex",
          flex: 1,
        },
      },
      h(
        "button",
        {
          className: "btn",
        },
        "Close"
      ),
      h(
        "button",
        {
          className: "btn unblock-btn",
        },
        "Unblock"
      )
    )
  );
}

export default Blocked;
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
