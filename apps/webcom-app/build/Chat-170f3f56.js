import { h, v, p, _ as _extends } from "./index-ddb7f63d.js";
...
const MessageObjectMappter = (props) => {
  ...
  return h(
    "div",
    null,
    dateSpace && h(DateLinebreak, props),
    h(
      MessageAligner,
      props,
      order === "F" ? h(FirstMessage, props) : h(SubsequentMessage, props)
    )
  );
};
  ...
  const mapMessages = () => {
    ...
    return messages.map((m, i) => {
      ...
      if (i > 0 && email === m.from && m.local) {
        if (
          new Date(lastDatetime).getDate() !== new Date(m.datetime).getDate()
        ) {
          email = m.from;
...
const MessagesDisplayer = ({
  ...
  const { mappedMessages } = useMessagesMapper(sortedMessages);
  return [
    h(
      MessageViewScroller,
      null,
      h(MessageCollectionView, {
...
function Settings({ height = 24, width = 24, fill = "none", color = "black" }) {
  return h(
    "svg",
    {
      height: height,
...
function Chat({ contact, onSetting }) {
